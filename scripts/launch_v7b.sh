#!/bin/bash
# Launch the two v7b selective-factor-edge arms sequentially on the A4000.
# Contract: configs/pilot-v7b-selective-factor-edge-v1-20260716.json
#   (SHA-256 ddac9f543cf5b71dda37e1a99a416570f6bb29096e2d9fd21678ca0009be0083)
# Both arms are the exact v6 fold3 command; the aligned arm adds the selective
# factor edges and the placebo arm adds the same edges with only the destination
# stock permuted. Sequential, because each arm peaks at ~10.8 GB of the 20 GB card.
set -u
...
--policy-rate-edge-scale 0.0 --event-edge-top-k 0 --event-edge-min-weight 0.05 \
...
--factor-sensitivity-top-k 25 --factor-sensitivity-min-abs-corr 0.15 \
--factor-sensitivity-mode signed --factor-sensitivity-scale 0.5"
}

PY=/root/venvs/stock-v2-cu128/bin/python

echo "=== ARM 1/2: aligned (real routing) $(date -u) ==="
$PY scripts/run_real_backtest.py $(common_args) \
  --reports-dir reports/pilot_v7b_factor_edge_aligned_seed17_20260716 \
  --models-dir models/pilot_v7b_factor_edge_aligned_seed17_20260716 \
  > logs/pilot_v7b_factor_edge_aligned_seed17_20260716.log 2>&1
echo "aligned exit=$? $(date -u)"

echo "=== ARM 2/2: placebo (permuted destinations) $(date -u) ==="
$PY scripts/run_real_backtest.py $(common_args) \
  --factor-sensitivity-permute-seed 43 \
  --reports-dir reports/pilot_v7b_factor_edge_placebo_seed17_20260716 \
  --models-dir models/pilot_v7b_factor_edge_placebo_seed17_20260716 \
  > logs/pilot_v7b_factor_edge_placebo_seed17_20260716.log 2>&1
echo "placebo exit=$? $(date -u)"

touch reports/PILOT_V7B_BOTH_ARMS_COMPLETE
