...
OUTPUT_DIR="$ROOT/reports/direct_state_mlp_${RUN_NAME}_20260714/fold1"
LOG="$ROOT/logs/direct_state_mlp_${RUN_NAME}_fold1_m1max_20260714.log"
...
printf '%s\n' \
  '{"scope":"research_only_direct_return_state_baseline","fold":"fold1","selection_data":"fit_and_validation_only","test_used_for_selection":false,"live_orders_allowed":false}' \
  > "$OUTPUT_DIR/experiment_contract.json"

if [[ -f "$OUTPUT_DIR/EXPERIMENT_COMPLETE" ]]; then
  printf '%s\n' "direct state fold1 baseline already complete"
  exit 0
fi
