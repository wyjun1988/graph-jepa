...
WAIT_ROOT="reports/impact_head_weight95_seed17_20260714"
...
OUTPUT_ROOT="reports/direct_state_impact_v1_20260714"
LOG_PATH="ops/training/direct_state_impact_m1pro_20260714.log"
PID_PATH="ops/training/direct_state_impact_m1pro_20260714.pid"
...
if [[ "${1:-}" != "--worker" ]]; then
  if [[ -f "$PID_PATH" ]] && kill -0 "$(cat "$PID_PATH")" 2>/dev/null; then
    echo "direct impact evaluation already queued: $(cat "$PID_PATH")"
    exit 0
  fi
  nohup caffeinate -dimsu bash "$0" --worker > "$LOG_PATH" 2>&1 < /dev/null &
  echo "$!" > "$PID_PATH"
  echo "direct impact evaluation queued: $!"
  exit 0
fi

while [[ ! -f "$WAIT_ROOT/IMPACT_COMPLETE" ]]; do
  if [[ -f "$WAIT_ROOT/IMPACT_FAILED" ]]; then
    echo "seed 17 impact run failed; refusing dependent direct comparison" >&2
    exit 4
  fi
  sleep 30
done
...
{
  date '+%Y-%m-%dT%H:%M:%S%z'
  shasum -a 256 scripts/evaluate_direct_state_impact.py "$0"
...
for fold in "$FOLD1" "$FOLD2"; do
  ...
  if [[ ! -f "$output_dir/summary.json" ]]; then
    "$PYTHON" scripts/evaluate_direct_state_impact.py \
      --model-dir "$MODEL_ROOT/${RUN_NAME}_${fold}" \
      --direct-graph-dir "$DIRECT_ROOT/direct/$fold" \
      --direct-nograph-dir "$DIRECT_ROOT/direct_nograph/$fold" \
...
printf '%s\n' \
  '{"scope":"read_only_direct_impact_challenger","live_orders_allowed":false}' \
