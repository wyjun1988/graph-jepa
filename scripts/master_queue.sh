#!/bin/bash
# A4000 master queue: keeps the GPU busy all night with zero idle gaps and no
# competing GPU-wait pollers (a second poller could fire in the gap between two
# W5 arms and contend for the card).
#
# Order: W5 sweep (already running, awaited via its marker)
#   -> v7 five-fold (contract fcc175c4..., preflight first)
#   -> H6-0 timing pilot (contract in configs/pilot-h60-timing-heads-v1-20260717.json)
#   -> v8 news+weight4 pilot (configs/pilot-v8-news-weight4-v1-20260717.json)
set -u
cd /workspace/stock-v2-pilot-v7
PY=/root/venvs/stock-v2-cu128/bin/python

echo "=== [1/4] waiting for the W5 sweep marker ==="
while [ ! -f /workspace/stock-v2-post-impact/reports/W5_H5B_COMPLETE ]; do sleep 120; done
echo "W5 done at $(date -u)"

echo "=== [2/4] v7 five-fold (news targets) ==="
bash scripts/launch_v7_5fold.sh > logs/v7_5fold_driver_rerun.log 2>&1
echo "v7 five-fold exit=$? $(date -u)"

pilot_args() {  # v7c base command for fold3; $1 = extra flags, $2 = tag
...
$1 \
--reports-dir reports/$2 --models-dir models/$2"
}

echo "=== [3/4] H6-0: timing heads on (aux 0.25, returns 12x kept) ==="
$PY scripts/run_real_backtest.py $(pilot_args "--downstream-auxiliary-loss-weight 0.25 \
...
--state-feature-weight intraday_return=12" pilot_h60_timing_seed17_20260717) \
  > logs/pilot_h60_timing_seed17_20260717.log 2>&1
echo "H6-0 exit=$? $(date -u)"

echo "=== [4/4] v8: news targets + weight4 ==="
$PY scripts/run_real_backtest.py $(pilot_args "--downstream-auxiliary-loss-weight 0.0 \
...
--state-feature-weight intraday_return=4" pilot_v8_news_weight4_seed17_20260717) \
  > logs/pilot_v8_news_weight4_seed17_20260717.log 2>&1
echo "v8 exit=$? $(date -u)"

echo "=== [5/5] plan-loss off-parity verification ==="
bash scripts/verify_plan_loss_offparity.sh

touch reports/MASTER_QUEUE_COMPLETE
echo "=== MASTER QUEUE DONE $(date -u) ==="
