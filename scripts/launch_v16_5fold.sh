...
RUN=v16_buysell_5fold_seed17_20260718
...
grep -q plan_buy_sell stock_v2/graph_jepa.py || { echo 'ABORT: hidden patch absent'; exit 1; }
echo "both data fixes + hidden patch present $(date -u)"
...
v11_args() {
  fold_args "$1" "$2" "$3" \
    ...
    | sed 's|$| --downstream-continuation-weight 1.0 --downstream-plan-loss-weight 0.25 --plan-temperature 0.01 --hidden-completion-weight 0.25 --plan-buy-sell|'
}
...
echo "$P" | grep -q -- '--hidden-completion-weight 0.25' || { echo 'ABORT: hidden weight missing'; exit 1; }
echo "$P" | grep -q -- '--plan-buy-sell' || { echo 'ABORT: buy-sell flag missing'; exit 1; }
echo "v16 5fold args verified: $(echo "$P" | wc -w) flags"
