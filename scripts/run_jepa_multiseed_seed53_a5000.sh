...
RUN_NAME="strict_causal453_path_v2_path_w12_p005_l025_skip_seed53"
REPORTS_ROOT="reports/walk_forward_causal453_path_multiseed_seed53_20260714"
MODELS_ROOT="models/walk_forward_causal453_path_multiseed_seed53_20260714"
LOG_PATH="ops/training/jepa_multiseed_seed53_a5000_20260714.log"
PID_PATH="ops/training/jepa_multiseed_seed53_a5000_20260714.pid"
...
if [[ "${1:-}" != "--worker" ]]; then
  if [[ -f "$PID_PATH" ]] && kill -0 "$(cat "$PID_PATH")" 2>/dev/null; then
    echo "seed 53 A5000 job already running: $(cat "$PID_PATH")"
    exit 0
  fi
  nohup bash "$0" --worker > "$LOG_PATH" 2>&1 < /dev/null &
  echo "$!" > "$PID_PATH"
  echo "seed 53 A5000 job started: $!"
  exit 0
fi

export PYTHONUNBUFFERED=1
export PYTHONHASHSEED=53
