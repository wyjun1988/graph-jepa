...
OUTPUT_ROOT="reports/direct_impact_equal_objective_m1pro_v1_20260714"
LOG_PATH="ops/training/direct_impact_equal_m1pro_20260714.log"
PID_PATH="ops/training/direct_impact_equal_m1pro_20260714.pid"
...
if [[ "${1:-}" != "--worker" ]]; then
  if [[ -f "$PID_PATH" ]] && kill -0 "$(cat "$PID_PATH")" 2>/dev/null; then
    echo "M1 Pro equal-objective direct impact job already running: $(cat "$PID_PATH")"
    exit 0
  fi
  nohup caffeinate -dimsu bash "$0" --worker > "$LOG_PATH" 2>&1 < /dev/null &
  echo "$!" > "$PID_PATH"
  echo "M1 Pro equal-objective direct impact job started: $!"
  exit 0
fi
...
{
  date '+%Y-%m-%dT%H:%M:%S%z'
  shasum -a 256 scripts/benchmark_direct_impact_head.py "$0"
  ...
  for mode in graph nograph; do
    ...
    if [[ ! -f "$output_dir/summary.json" ]]; then
      "$PYTHON" scripts/benchmark_direct_impact_head.py \
        ...
        --context-cache "data/context_cache/direct_impact_equal_m1pro_${short_fold}_20260714.npy" \
        $extra_arg
    fi
  done
done
...
printf '%s\n' \
  '{"scope":"read_only_equal_objective_direct_challenger_m1pro","live_orders_allowed":false}' \
