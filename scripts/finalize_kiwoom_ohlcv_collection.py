...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze verified Kiwoom daily outputs into an immutable release."
    )
...
def main() -> int:
    ...
    tickers = _universe_tickers(universe_path)
    records: dict[str, dict[str, Any]] = {}
    for line in coverage_path.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        record = json.loads(line)
        if str(record.get("run_id")) != args.run_id or str(record.get("basis")) != args.basis:
            continue
    ...
    try:
        for ticker in tickers:
            ...
            if status == "ok":
                ...
                relative = Path("outputs") / source.name
                temporary_destination = temporary_dir / relative
                ...
                final_destination = output_dir / relative
                ...
                if record.get("output_sha256") is not None:
                    raise ValueError(f"empty record has output checksum: {ticker}")
            else:
        ...
        manifest = {
            "schema_version": 1,
            "collection_contract": "immutable_kiwoom_ohlcv_collection_v1",
            "source": "kiwoom_rest_ka10081",
            "run_id": args.run_id,
