set -u
cd /workspace/stock-v2-pilot-v7
PY=/root/venvs/stock-v2-cu128/bin/python
RUN=v11_fixeddata_seed17_20260717

# v11 is v9's CONFIGURATION on a corrected panel. That is what makes it a
# measurement: hold the configuration fixed, change only the data, and the delta
# against v9 is what the defective bars were worth.
#
# Refuse to spend GPU unless BOTH fixes are actually here. A v11 trained on the
# old panel would be v9 with a new name and would answer nothing.
grep -q 'carried flat' stock_v2/market_data.py || { echo 'ABORT: suspension fix absent'; exit 1; }
grep -q '_discrete_quarters' stock_v2/fundamental_features.py || { echo 'ABORT: accounting-basis fix absent'; exit 1; }
echo "both data fixes present $(date -u)"

check_quota() {
  dd if=/dev/zero of=/workspace/_q.bin bs=1M count=400 2>/dev/null
  local sz; sz=$(stat -c%s /workspace/_q.bin 2>/dev/null || echo 0)
  rm -f /workspace/_q.bin
  if [ "$sz" -lt 419430400 ]; then
    echo "ABORT: /workspace truncates writes ($sz/419430400)"
    df -h /workspace | tail -1
    exit 1
  fi
}
check_quota; echo "write check OK"

sed -n '/^fold_args() {/,/^}/p' scripts/launch_v7_5fold.sh \
  | sed "s|reports/\${RUN}|reports/${RUN}|g; s|models/\${RUN}|models/${RUN}|g" > /tmp/v11.sh
source /tmp/v11.sh

# v9's flags exactly: v7's fold_args plus aux/continuation/plan. The mask stays
# `mixed`. Every substitution is asserted before a single epoch runs.
v11_args() {
  fold_args "$1" "$2" "$3" \
    | sed 's|--downstream-auxiliary-loss-weight 0.0 |--downstream-auxiliary-loss-weight 0.25 |' \
    | sed 's|$| --downstream-continuation-weight 1.0 --downstream-plan-loss-weight 0.25 --plan-temperature 0.01|'
}
P=$(v11_args 2024-11-05 2024-01-03 r3)
echo "$P" | grep -q -- '--mask-strategy mixed ' || { echo 'ABORT: v9 mask not preserved'; exit 1; }
echo "$P" | grep -q -- '--mask-strategy operational_mixed' && { echo 'ABORT: v10 mask leaked in'; exit 1; }
echo "$P" | grep -q -- '--downstream-continuation-weight 1.0' || { echo 'ABORT: continuation flag lost'; exit 1; }
echo "$P" | grep -q -- '--downstream-auxiliary-loss-weight 0.25' || { echo 'ABORT: aux weight lost'; exit 1; }
echo "$P" | grep -q -- '--max-tickers 500' || { echo 'ABORT: fold_args broken'; exit 1; }
echo "$P" | grep -q "models/${RUN}/r3" || { echo "ABORT: path substitution wrong"; exit 1; }
echo "v11 args verified: $(echo "$P" | wc -w) flags"

mkdir -p logs models/${RUN}
echo "--- TRAIN v11 r3 (corrected panel) $(date -u) ---"
$PY scripts/run_real_backtest.py $P > logs/${RUN}_r3.log 2>&1
echo "train exit=$? $(date -u)"
grep -E '^epoch' logs/${RUN}_r3.log | tail -1
