...
def _assert_checkpoint_args(
    ...
    for name, value in expected.items():
        ...
        if isinstance(value, float):
            ...
            raise ValueError(f"checkpoint arg mismatch: {name}")
    if not bool(args.get("disable_stale_graph", False)) or bool(
        args.get("permute_stale_graph_nodes", False)
    ):
        raise ValueError(f"checkpoint scalar graph flags mismatch: {mode}")
    if str(args.get("graph_message_mode", "none")) != mode:
        raise ValueError(f"checkpoint graph message mode mismatch: {mode}")


def evaluate(contract_path: Path) -> tuple[dict[str, Any], pd.DataFrame]:
    contract = json.loads(contract_path.read_text(encoding="utf-8"))
    if contract.get("role") != "post_impact_sparse_message_ablation_contract":
        raise ValueError("invalid sparse-message ablation contract role")
    if contract.get("live_orders_allowed") is not False:
        raise ValueError("unsafe sparse-message contract")
    if contract.get("retrospective_period_previously_inspected") is not True:
        raise ValueError("sparse-message contract must disclose test reuse")
    ...
    for name, spec in contract["models"].items():
        ...
        if summary.get("stale_stock_graph_mode") != "disabled":
            raise ValueError(f"scalar graph sensor was not disabled: {name}")
        if summary.get("graph_message_mode") != spec["graph_message_mode"]:
            raise ValueError(f"training graph message mode mismatch: {name}")
        if summary.get("graph_message_input_enabled") is not True:
            raise ValueError(f"graph message architecture is absent: {name}")
        expected_edges = spec["graph_message_mode"] != "disabled"
        if summary.get("graph_message_edges_used") is not expected_edges:
            raise ValueError(f"graph message edge-use mismatch: {name}")
        ...
        parameter_counts.add(int(summary["runtime"]["model_parameters"]))
    ...
    if len(split_records) != 1:
        raise ValueError("sparse-message models do not share identical splits")
    if len(parameter_counts) != 1:
        raise ValueError("sparse-message controls do not share one architecture")
    ...
    for comparator in comparator_names:
        ...
        endpoint_pearson_deltas = np.asarray(
            [
                full_session[horizon][comparator]["pearson"]["mean_delta"]
                for horizon in HORIZONS
            ],
            dtype=np.float64,
        )
        checks[f"endpoint_mean_pearson_vs_{comparator}"] = float(
            endpoint_pearson_deltas.mean()
        ) > float(gates["minimum_endpoint_mean_pearson_delta"])
        checks[f"endpoint_positive_horizons_vs_{comparator}"] = int(
            (endpoint_pearson_deltas > 0.0).sum()
        ) >= int(gates["minimum_endpoint_positive_horizons"])
    ...
    summary = {
        "schema_version": 1,
        "role": "post_impact_sparse_message_ablation_audit",
        ...
        "model_parameters": int(next(iter(parameter_counts))),
        "retrospective_period_previously_inspected": True,
        "decision": (
            "sparse_graph_message_increment_confirmed_for_new_period_validation"
            if passed
            else "sparse_graph_message_increment_not_confirmed"
        ),
        "next_gate": (
            "prospective_read_only_shadow_on_new_unseen_sessions"
            if passed
            else "do_not_promote_sparse_graph_message_candidate"
        ),
        "inputs": inputs,
        "status": "complete",
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit identity-preserving sparse graph-message increment."
    )
