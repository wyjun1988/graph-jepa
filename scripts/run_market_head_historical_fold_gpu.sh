...
FOLD_ID="${1:?fold id is required, for example meta2022}"
TRAIN_END="${2:?train end is required}"
END_DATE="${3:?panel end is required}"
MODE="${4:-manifest}"
EXPECTED_DATA_SHA256="${EXPECTED_DATA_SHA256:-}"
EXPECTED_EDGE_SHA256="${EXPECTED_EDGE_SHA256:-}"
MIN_EVENT_COVERAGE="${MIN_EVENT_COVERAGE:-0.95}"
MIN_FUNDAMENTAL_COVERAGE="${MIN_FUNDAMENTAL_COVERAGE:-0.86}"
MIN_INVESTOR_COVERAGE="${MIN_INVESTOR_COVERAGE:-0.89}"
RUN_NAME="final_aligned_jepa_market_head_historical_v1_seed17"
FOLD_NAME="${RUN_NAME}_${FOLD_ID}_${TRAIN_END//-/}_to_${END_DATE//-/}"
...
case "$MODE" in
  ...
  train)
    ...
    ;;
  *)
    echo "mode must be manifest, edge-manifest, or train" >&2
    exit 2
    ;;
esac

printf '{"scope":"research_only","live_orders_allowed":false,"purpose":"historical OOS meta-head training","fold_id":"%s","train_end":"%s","panel_end":"%s","minimum_event_coverage":%s,"minimum_fundamental_coverage":%s,"minimum_investor_coverage":%s}\n' \
  "$FOLD_ID" "$TRAIN_END" "$END_DATE" "$MIN_EVENT_COVERAGE" \
  "$MIN_FUNDAMENTAL_COVERAGE" "$MIN_INVESTOR_COVERAGE" \
