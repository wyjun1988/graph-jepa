...
RUN=v15_privileged_teacher_seed17_20260718
...
grep -q 'attach_hidden_completion_targets' scripts/run_real_backtest.py || { echo 'ABORT: hidden-completion patch absent'; exit 1; }
grep -q 'install_privileged_hidden' scripts/run_real_backtest.py || { echo 'ABORT: privileged-teacher patch absent'; exit 1; }
...
v14_args() {
  fold_args "$1" "$2" "$3" \
    ...
    | sed 's|$| --downstream-continuation-weight 1.0 --downstream-plan-loss-weight 0.25 --plan-temperature 0.01 --hidden-completion-weight 0.25 --privileged-hidden-teacher|'
}
...
echo "$P" | grep -q -- '--privileged-hidden-teacher' || { echo 'ABORT: privileged teacher flag not applied'; exit 1; }
...
echo "v15 args verified: $(echo "$P" | wc -w) flags -- one change from v11"

mkdir -p logs models/${RUN}
echo "--- TRAIN v15 r3 (hidden head + privileged teacher) $(date -u) ---"
