...
#
# ── 동시 실행 (선택) ─────────────────────────────────────────────────────
#   CONCURRENCY=2 bash scripts/4090_queue.sh
#
# 실측(A5000): 런당 VRAM 11.8GB(nvidia-smi, CUDA 컨텍스트 포함), 41~45분.
#   분해 = 데이터로딩 ~7분 + 학습 26분(24에폭x65초) + 평가 ~11분
# 24GB 에서 2개면 23.6GB 로 여유가 약 1GB 뿐이다. 게다가 학습 중 GPU 사용률이
# 100% 라 SM 을 나눠 쓰게 되어 2배가 나오지 않는다 — 겹쳐서 이득이 나는 구간은
# CPU 쪽(로딩·평가 일부)이라 기대 이득은 1.3~1.4배다.
# 중간 OOM 이 나면 30분 이상을 버리므로 기본값은 1 이다.
# VRAM 26GB 이상이면 아래 점검이 2를 허용한다.
#
# 워커 수는 WORKERS 로 덮어쓸 수 있다 (기본 = 코어/2).
#   예) WORKERS=24 bash scripts/4090_queue.sh
#
# 예상 소요: 6런(2폴드 x 3시드) x 35~45분 = 4~5시간 (4090 은 더 짧을 수 있음)
...
for F in r2 r1; do
  echo "──────── 폴드 $F ────────"
  PREFIX=ens_s bash scripts/seed_queue_v2.sh "$CONCURRENCY" "$F" 3 17 29
done
